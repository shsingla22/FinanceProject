"""
Line graph of Nifty Midcap 100 year-end closing values plus yearly
percentage change, sourced only from nifty_midcap100_data.csv.

Covers calendar years 2006-2025. The percentage-change series starts
from 2007 because 2005 isn't in the source data (the index has data
back to 2003 but SEBI Annual Reports only published monthly Dec values
from 2006 onwards; earlier values are accessible from niftyindices.com
via browser).
"""
...
CSV = HERE / "nifty_midcap100_data.csv"
OUT = HERE / "nifty_midcap100_close_vs_yoy.png"
...
def plot() -> None:
    ...
    ax_close.set_ylabel("Year-end closing value", color="#9467bd")
    ax_pct.set_ylabel("YoY % change", color="#d62728")
    ax_close.tick_params(axis="y", labelcolor="#9467bd")
    ...
    ax_close.set_title(
        "Nifty Midcap 100 — year-end close and YoY % change (CY 2006-2025)\n"
        "Source: SEBI Annual Reports + SEBI Handbook 2015 + NSE archives end-of-day index bhavcopy",
        fontsize=12,
    )
