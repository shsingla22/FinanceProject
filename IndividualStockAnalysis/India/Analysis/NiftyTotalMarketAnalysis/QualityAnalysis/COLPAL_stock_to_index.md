...
The same graph in two other formats sits beside this file: [`COLPAL_stock_to_index.svg`](COLPAL_stock_to_index.svg) — a vector chart that opens in any browser — and [`COLPAL_stock_to_index.mmd`](COLPAL_stock_to_index.mmd) — Mermaid source, which draws itself when pasted into GitHub.
